AspectRatioButton = Button:extend()
AspectRatioButton.list = {
    '16:9',
    '16:10',
    '21:9',
}

function AspectRatioButton:new(owner, w, h, tag)
    AspectRatioButton.super.new(self, owner, w, h, tag)
    self.selected = false
    self.active_character = ASPECT_RATIO
end


function AspectRatioButton:update(dt)
    if self.active then
        self.h = height/9
    else
        self.h = height/15
    ...
    AspectRatioButton.super.update(self, dt)
end


function AspectRatioButton:draw_arrow(x, y, r)
    local size = self.h/2
    graphics.push()
...
function AspectRatioButton:draw(x, y)    
    graphics.push()
...
function AspectRatioButton:switch_right()
    self.active_character = self.active_character + 1
    if self.active_character > #self.list then
        self.active_character = 1
    end
    ASPECT_RATIO = self.active_character
    self:change_res()
end

function AspectRatioButton:switch_left()
    self.active_character = self.active_character - 1
...
function AspectRatioButton:change_res()
    if self.active_character == 1 then -- 16:9
    ...
    elseif self.active_character == 2 then -- 16:10
        if RESOLUTION_QUALITY == 1 then
    ...
    elseif self.active_character == 3 then -- 21:9
        if RESOLUTION_QUALITY == 1 then
    ...
    gamestate.switch(Home, TYPE.OPTIONS, 2)
end
...
-- events = random:chance_list({'X', 2}, {'Y', 2}, {'Z', 4})
-- for i = 1, 16 do
--     print(events:pop())
-- end
function Random:chance_list(...)
	return {
		chance_list = {},
		chance_candidates = { ... },
		pop = function(self)
			if #self.chance_list == 0 then
				for _, candidate in ipairs(self.chance_candidates) do
					for i = 1, candidate[2] do table.insert(self.chance_list, candidate[1]) end
				end
			end
			return table.remove(self.chance_list, random.generator:random(1, #self.chance_list))
		end,
	}
end
